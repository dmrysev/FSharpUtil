module Util.IO.FileSystem.Default.DataAccess
...
let openImageFile (filePath: FilePath) =
    let command = $"feh -ZFYr --action9=\"mv %%F /mnt/data/trash\" '{filePath.Value}' &"
    Util.Process.executeNoOutput command

let openImageFiles (temporaryDataDirPath: DirectoryPath) (filePaths: FilePath seq) =
    let playlistFilePath = createPlaylistFile temporaryDataDirPath filePaths
    let command = $"(feh -ZFYr --action9=\"mv %%F /mnt/data/trash\" -f '{playlistFilePath.Value}';rm '{playlistFilePath.Value}')&"
    Util.Process.executeNoOutput command
...
let init (appDataDirPath: DirectoryPath) =
    ...
    let dataAccess: API.FileSystem.DataAccess = {
        File = {|
